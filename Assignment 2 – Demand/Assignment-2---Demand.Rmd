...
where the products $j$, markets $t$, and characteristics $l$ are unchanged from the previous section. While it is more reasonable to assume that none of the utility weights are truly uniform across individuals, this would complicate calculations perhaps more than it is worth. The variables to which it seems particularly important to add random coefficients are to the luxury and compact class types (hoping that the middle types are standard enough to face a smaller range of random coefficients), because the car classes probably capture much of the heterogeneity in other features. Other important variables are given constant coefficients in order to reduce computing time.
...
contraction_mapping <- function(sigma){
  ...
  mu <- as.matrix(ungroup(cars) %>% select(cla_2, cla_5)) %*% t(sigmanu)
...
Now we program the GMM function:

```{r}
gmm <- function(theta){
  
  # Creating delta(sigma)
  delta <- contraction_mapping(c(theta[1], theta[2]))
  
  # Creating xi = delta - x*beta -p*alpha
  xi <- delta - X %*% c(theta[3:(ncol(X) + 2)])
  
  # Creating V and then W as its inverse
  V <- matrix(NA, nrow = ncol(Z), ncol = ncol(Z))
  for(i in 1:ncol(Z)){
    for(j in 1:ncol(Z)){
      V[i, j] = mean((Z[,i] * xi) * (Z[,j] * xi))
    }
  }
  W <- inv(V)
...
The following block attempts to minimize the GMM objective function. I constrain the $\sigma$s to be non-negative, and I also constrain them to be no more than twice as large as the standard errors of their corresponding coefficients in the IV estimation. This is necessary because trying larger values can prevent the function from running (R rounds very large numbers in the denominator of the integral to infinity), and I believe that twice the size of the original standard errors is a reasonable bound on the $\sigma$s.

```{r, warning=FALSE, message=FALSE}
# First running IV again with fixed effects, just for the starting guess of coefficients
IV2 <- feols(log(sj) - log(s0) ~ hp + we + li + sp + factor(cla) + home + factor(brd) | princ ~ competition, cars, cluster = c("ye", "brd"))
...
ci <- c(0, 0, -IV2$se[c(7,10)] * 2)

theta <- constrOptim(
  theta = c(IV2$se[c(7,10)], IV2$coefficients[2:length(IV2$coefficients)]),  # initial guess is IV parameters
  f = gmm,
  ui = ui,
  ci = ci,
  method = "Nelder-Mead",
  control = list(maxit = 1) # Note: in one iteration there are multiple evaluations, so we do not end up with exactly our initial guess
)
```
...
```{r}
# Replicating delta(sigma) (needed for the next step)
delta <- contraction_mapping(c(theta$par[1:2]))
...
moments <- function(par){
  sapply(as_tibble(Z) * (delta - X %*% c(par[1:(ncol(X))])), mean)
}
...
for(i in 1:ncol(Z)){
  for(j in 1:ncol(Z)){
    V[i, j] = mean((Z[,i] * (delta - X %*% theta$par[3:(ncol(X) + 2)])) * (Z[,j] * (delta - X %*% theta$par[3:(ncol(X) + 2)])))
  }
}
...
##### Question 7
Unfortunately my estimate of $D'V^{-1}D$ is currently singular, and so I do not yet have standard errors for the estimates. But just looking at the coefficients themselves, the results currently do not necessarily look more credible than the logit model's estimates. We can see that price still has a negative coefficient, but that the sizes of the class types' utility weights are unreasonable relative to each other. This could be due to a poor methodological choice of random coefficients, or possibly also due to my limiting of the optimization function's number of iterations.

The random coefficients appear reasonable (although, again, I cannot comment on their significance). They are proportioned relative to each other similarly to the IV estimate's standard errors, and are both larger than them.

```{r}
# Mean utility weights
theta$par[3:length(theta$par)]
