...
Organizing the various data objects into a single tibble:

```{r}
# Auxiliary object to create the post-FTA marginal costs
...
Trial with lower prices:

```{r}
temp_prices <- prices_post
temp_prices[newdata$Korea_ind == TRUE] <- prices_post[newdata$Korea_ind == TRUE]

# Updating mean utility in data
temp_delta_data <- data.frame(
  product_id = BLPdata$parameters$product_id,
  market_id = BLPdata$parameters$market_id_char_in,
  delta = shareObj$shares,
  const_price = temp_prices
)

# Updating BLP_data object to new data
temp_BLPdata <- update_BLP_data(
  data_update = temp_delta_data,
  blp_data = BLPdata_updated
)

# Adjusting the relevant information inside final_BLPdata
temp_BLPdata$data$X_rand[,2] <- temp_BLPdata$data$X_lin[,2]

# Calculating sij
temp_shareObj <- getShareInfo(
  blp_data = temp_BLPdata,
  par_theta2 = temp_theta2_all,
  printLevel = 0
)
...
c(
  ...
  mean(mc_pre_yevuan),
  median(mc_pre),
  median(mc_pre_yevuan),
  cor(mc_pre, mc_pre_yevuan)
)
c(
  sum(mc_pre > 0),
  sum(mc_pre_yevuan > 0),
...
view(cbind(prices_pre, mc_pre, mc_pre_yevuan, mc_pre - mc_pre_yevuan))
```
...
# Table 2: Regression Outputs – simple logit 1st and 2nd stages, and RCL
...
# Table 4: Prices, MCs, markups, for the 6 models
...
prices_pre[BLPdata$parameters$product_id == 10245]
